...
namespace sistema.Controllers
{
    ...
    public class BannerController : Controller
    {
        ...
        public BannerController(AppIdentityDbContext context,UserManager<AppUser> userMgr, SignInManager<AppUser> signinMgr)
        {
            _context = context;
            userManager   = userMgr;
    ...
    [Authorize(Roles = "Administrador, Revenda, Parceiro, Cliente")]
        ...
        public async Task<IActionResult> Index(string? Categoria, string? Titulo, string? Inicio, string? Fim)
        {
            ...
            ViewBag.INFOTITLE = "INFORMAÇÕES SOBRE BANNERS";
            ViewBag.INFOTEXT  = "Administradores. Esta área permite ao super-admin gestoriar e cadastrar novos banners ao website que fica na área principal do sistema.";
            ...
            var GradeQuery = from gq in _context.Banner orderby gq.codigo select gq.date;
            GradeList.AddRange(GradeQuery.Distinct());
            ViewBag.Clientes__Grade = new SelectList(GradeList);

            var KY = from cr in _context.Banner select cr;
            ...
            {
                datalogs_.DATALOG(HASHCODE, DATE, "ADMINISTRADOR", @User.Identity.Name, Text.FILTRO() + " BANNER", location_.GetLocalIPAddress());
        ...
        public IActionResult Novo()
        {
            ...
            ViewBag.INFOTITLE = "INFORMAÇÕES SOBRE CADASTRO DE BANNERS";
            ViewBag.INFOTEXT  = "Administradores. Permite ao administrador cadastrar novos banners ao website, que ficam no topo do website";
            ViewBag.INFOBUTTON = "INFORMAÇÕES";
        ...
        public async Task<IActionResult> Novo(IFormFile foto, Banner dados)
        {
            ...
            try
            {
                ...
                string SavePath   = Path.Combine(Directory.GetCurrentDirectory(),IpURI.BANNER(),ImageName);

                using (var stream = new FileStream(IpURI.BANNER() + HASHCODE + ".png", FileMode.OpenOrCreate))

                {
                    ...
                    datalogs_.DATALOG(HASHCODE, DATE, "ADMINISTRADOR", @User.Identity.Name, Text.SAVE() + " BANNER", location_.GetLocalIPAddress());
        ...
        public async Task<IActionResult> Dados(int? id, string? hash)
        {
            ...
            ViewBag.INFOTITLE = "INFORMAÇÕES SOBRE CADASTRO DE BANNERS";
            ViewBag.INFOTEXT  = "Administradores. Permite ao administrador atualizar banners ao website, que ficam no topo do website";
            ViewBag.INFOBUTTON = "INFORMAÇÕES";
            ...
            var dados = await _context.Banner.FindAsync(id);
            if (dados == null)
            {
                return NotFound();
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Dados(IFormFile foto,int id, Banner dados)
        {
            ...
            if(foto != null){
                    ...
                    using (var stream = new FileStream(IpURI.BANNER() + HASHCODE + ".png", FileMode.OpenOrCreate))

                        {
                            ...
                            await _context.SaveChangesAsync();
                            datalogs_.DATALOG(HASHCODE,DATE,"ADMINISTRADOR",@User.Identity.Name,Text.UPDATE() + " BANNER", location_.GetLocalIPAddress());    
                        }
        ...
        public async Task<IActionResult> Delete(int? id)
        {
            ...
            var dados = await _context.Banner
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            if (dados!= null)
            {
                _context.Banner.Remove(dados);
                datalogs_.DATALOG(HASHCODE,DATE,"ADMINISTRADOR",@User.Identity.Name,Text.DELETE() + " BANNER", location_.GetLocalIPAddress());        
            }
        ...
        {
          return (_context.Banner?.Any(e => e.codigo == id)).GetValueOrDefault();
        }
