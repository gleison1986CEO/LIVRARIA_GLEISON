...
namespace sistema.Controllers
{
    ...
    public class ConsoleController : Controller
    {
        ...
        public ConsoleController(AppIdentityDbContext context,UserManager<AppUser> userMgr, SignInManager<AppUser> signinMgr)
        {
            _context = context;
            userManager   = userMgr;
    ...
    [Authorize(Roles = "Administrador, Revenda, Parceiro, Cliente")]
        ...
        public async Task<IActionResult> Index(string? Console, string? Titulo, string? Inicio, string? Fim)
        {
            ...
            ViewBag.INFOTITLE = "INFORMAÇÕES SOBRE CONSOLES";
            ViewBag.INFOTEXT  = "Administradores. Permite ao administrador cadastrar novos Console e revisar Consoles anexadas aos filmes, canais, séries e eoutros";
            ViewBag.INFOBUTTON = "INFORMAÇÕES";
                
            // INFORMAÇÕES POPUP  
            // SEARCH
            var GradeList = new List<string>();
            var GradeQuery = from gq in _context.Consoles orderby gq.codigo select gq.date;
            GradeList.AddRange(GradeQuery.Distinct());
            ViewBag.Clientes__Grade = new SelectList(GradeList);

            var KY = from cr in _context.Consoles select cr;
            ...
            if (!String.IsNullOrEmpty(Console))
            {
                ...
                KY = KY.Where(g => g.nome.Contains(Console));
            ...
            {
                datalogs_.DATALOG(HASHCODE, DATE, "ADMINISTRADOR", @User.Identity.Name, Text.FILTRO() + " CONSOLE", location_.GetLocalIPAddress());
        ...
        public IActionResult Novo()
        {
            ...
            ViewBag.INFOTITLE = "INFORMAÇÕES SOBRE CADASTRO DE CONSOLE";
            ViewBag.INFOTEXT  = "Administradores. Permite ao administrador cadastrar Console para o website.";
            ViewBag.INFOBUTTON = "INFORMAÇÕES";
        ...
        public async Task<IActionResult> Novo(Consoles dados)
        {
            ...
            try
            {
                ...
                datalogs_.DATALOG(HASHCODE, DATE, "ADMINISTRADOR", @User.Identity.Name, Text.SAVE() + " CONSOLE", location_.GetLocalIPAddress());
        ...
        public async Task<IActionResult> Dados(int? id, string? hash)
        {
            ...
            ViewBag.INFOTITLE = "INFORMAÇÕES SOBRE CADASTRO DE CONSOLE";
            ViewBag.INFOTEXT  = "Administradores. Permite ao administrador atualizar Console para o website.";
            ViewBag.INFOBUTTON = "INFORMAÇÕES";
            ...
            var dados = await _context.Consoles.FindAsync(id);
            if (dados == null)
            {
                return NotFound();
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Dados(int id, Consoles dados)
        {
            ...
            if (id != dados.codigo)
            {
                ...
                try
                {
                        ...
                        await _context.SaveChangesAsync();
                        datalogs_.DATALOG(HASHCODE,DATE,"ADMINISTRADOR",@User.Identity.Name,Text.UPDATE() + " CONSOLE", location_.GetLocalIPAddress());    
              
                }
                catch (DbUpdateConcurrencyException)
                {
                    if (!ConsolesExists(dados.codigo))
                    {
                        return NotFound();
                    }
                    else
        ...
        public async Task<IActionResult> Delete(int? id)
        {
            ...
            // INFORMAÇÕES POPUP
            ViewBag.INFOTITLE = "INFORMAÇÕES SOBRE CONSOLE";
            ...
            var dados = await _context.Consoles
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            {
                return Problem("Entity set 'Context.Consoles'  is null.");
            }
            var dados = await _context.Consoles.FindAsync(id);
            if (dados!= null)
            {
                _context.Consoles.Remove(dados);
                datalogs_.DATALOG(HASHCODE,DATE,"ADMINISTRADOR",@User.Identity.Name,Text.DELETE() + " CONSOLE ", location_.GetLocalIPAddress());        
            }
        ...
        {
          return (_context.Consoles?.Any(e => e.codigo == id)).GetValueOrDefault();
        }
